...
describe('Keystore', function () {
  ...
  describe('generateNewAddressOnPath', function () {
    var specificHDpath = fixtures.CustomPath[0].specificHDpath;
    var N = fixtures.CustomPath[0].addresses.length;
    it('returns a new address, using custom path', function (done) {
      this.timeout(10000);
      for (var i = 0; i < N; i++) {
        var ks = new keyStore(fixtures.CustomPath[0].mnSeed, Uint8Array.from(fixtures.CustomPath[0].pwDerivedKey), specificHDpath);
        ks.addHdDerivationPath(specificHDpath, Uint8Array.from(fixtures.CustomPath[0].pwDerivedKey), {curve: 'secp256k1', purpose: 'sign'});
        ks.generateNewAddressOnPath(Uint8Array.from(fixtures.CustomPath[0].pwDerivedKey), specificHDpath, fixtures.CustomPath[0].addresses[i].index);
        var addresses = ks.getAddresses();
        var addr = addresses[0];
        var priv = ks.exportPrivateKey(addr, Uint8Array.from(fixtures.CustomPath[0].pwDerivedKey), specificHDpath);
        expect(addr).to.equal(fixtures.CustomPath[0].addresses[i].address);
        expect(priv).to.equal(fixtures.CustomPath[0].addresses[i].privKeyHex);
      }
      done();
    });
  });
