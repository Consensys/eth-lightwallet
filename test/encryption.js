...
describe("Encryption", function () {
  ...
  describe('Multi-recipient Encryption', function() {
    ...
    it('encrypts and decrypts a string to multiple parties', function (done) {
      ...
      var encrypted = encryption.multiEncryptString(msg, pubKeys.slice(0,4));
      var cleartext = encryption.multiDecryptString(ks, pw, encrypted, pubKeys[0], hdPath);
      expect(cleartext).to.equal(msg);
      cleartext = encryption.multiDecryptString(ks, pw, encrypted, pubKeys[1], hdPath);
      expect(cleartext).to.equal(msg);
      cleartext = encryption.multiDecryptString(ks, pw, encrypted, pubKeys[2], hdPath);
      expect(cleartext).to.equal(msg);
      cleartext = encryption.multiDecryptString(ks, pw, encrypted, pubKeys[3], hdPath);
      expect(cleartext).to.equal(msg);
      cleartext = encryption.multiDecryptString(ks, pw, encrypted, pubKeys[4], hdPath);
