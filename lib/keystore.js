...
KeyStore.prototype.addHdDerivationPath = function (hdPathString, password, info) {
  ...
  if (info.purpose !== 'sign' && info.purpose !== 'encrypt') {
    throw new Error("KeyStore.addHdDerivationPath: info.purpose is '" + info.purpose + "' but must be either 'sign' or 'encrypt'.");
  }
...
KeyStore.deserialize = function (keystore) {
  ...
  if (jsonKS.ksData === undefined) {
    ...
    // TODO: make this into separate one?

    defaultHdPathString = "m/0'/0'/0'";
    keystoreX.ksData = {};
    keystoreX.ksData[defaultHdPathString] = {};
    keystoreX.ksData[defaultHdPathString].encHdPathPriv = jsonKS.encHdPathPriv;
    keystoreX.ksData[defaultHdPathString].hdIndex       = jsonKS.hdIndex;
    keystoreX.ksData[defaultHdPathString].encPrivKeys   = jsonKS.encPrivKeys;
    keystoreX.ksData[defaultHdPathString].addresses     = jsonKS.addresses;
...
KeyStore.prototype.generateNewEncryptionKeys = function(password, n, hdPathString) {
  ...
  if (this.ksData[hdPathString].info.purpose !== 'encrypt') {
    throw new Error('KeyStore.generateNewEncryptionKeys: Address not defined when purpose is not "encrypt"');
  }
...
KeyStore.prototype.getPubKeys = function (hdPathString) {
  ...
  if (this.ksData[hdPathString].info.purpose !== 'encrypt') {
    throw new Error('KeyStore.getPubKeys: Not defined when purpose is not "encrypt"');
  }
...
KeyStore.prototype._encryptRaw = function (msgUint8Array, myPubKey, theirPubKey, password, hdPathString) {
  ...
  if (this.ksData[hdPathString].info.purpose !== 'encrypt') {
    throw new Error('KeyStore.encryptString: Function not defined when purpose is not "encrypt"');
  }
  
  if (this.ksData[hdPathString].encPrivKeys[myPubKey] === undefined) {
    throw new Error('KeyStore.encryptString: public key not found in KeyStore');
  }
  ...
  var privKey = KeyStore._decryptKey(encPrivKey, encKey);
  var privKeyBase64 = (new Buffer(privKey, 'hex')).toString('base64');
  var pubKeyBase64 = (new Buffer(theirPubKey, 'hex')).toString('base64');
  var privKeyUInt8Array = nacl.util.decodeBase64(privKeyBase64);
  var pubKeyUInt8Array = nacl.util.decodeBase64(pubKeyBase64);
...
KeyStore.prototype._decryptRaw = function (encMsg, theirPubKey, myPubKey, password, hdPathString) {
  ...
  if (this.ksData[hdPathString].info.purpose !== 'encrypt') {
    throw new Error('KeyStore.decryptString: Function not defined when purpose is not "encrypt"');
  }
  
  if (this.ksData[hdPathString].encPrivKeys[myPubKey] === undefined) {
    throw new Error('KeyStore.decryptString: public key not found in KeyStore');
  }
...
KeyStore.prototype.encryptString = function (msg, myPubKey, theirPubKey, password, hdPathString) {

  var messageUInt8Array = nacl.util.decodeUTF8(msg);

  return this._encryptRaw(messageUInt8Array, myPubKey, theirPubKey, password, hdPathString);

}

KeyStore.prototype.decryptString = function (encMsg, theirPubKey, myPubKey, password, hdPathString) {

  var cleartext = this._decryptRaw(encMsg, theirPubKey, myPubKey, password, hdPathString);
