...
KeyStore.prototype._generatePrivKeyOnPath = function (pwDerivedKey, hdPathString, index) {
  if (!this.isDerivedKeyCorrect(pwDerivedKey)) {
    throw new Error('Incorrect derived key!');
  }
  ...
  var hdRoot = KeyStore._decryptString(this.ksData[hdPathString].encHdPathPriv, pwDerivedKey);
  console.log('hdroot', hdRoot);
  if (hdRoot.length === 0) {
    throw new Error('Provided password derived key is wrong');
  }

  var hdprivkey = new bitcore.HDPrivateKey(hdRoot).derive(index);
  var privkeyBuf = hdprivkey.privateKey.toBuffer();
...
KeyStore.prototype.generateNewAddressOnPath = function (pwDerivedKey, hdPathString, index) {
  if (!this.isDerivedKeyCorrect(pwDerivedKey)) {
    throw new Error('Incorrect derived key!');
  }
  ...
  var key = this._generatePrivKeyOnPath(pwDerivedKey, hdPathString, index);

  var address = KeyStore._computeAddressFromPrivKey(key.privKey);
  this.ksData[hdPathString].encPrivKeys[address] = key.encPrivKey;
  this.ksData[hdPathString].addresses.push(address);
  var keyObj = {address: address,
    encPrivkey: key.encPrivKey,
    PrivKey: key.privKey};
  return keyObj;
};
