...
var KeyStore = function(mnemonic, pwDerivedKey, hdPathString) {
  ...
  this.defaultHdPathString = "m/44'/60'/0'/0";
...
KeyStore.deserialize = function (keystore) {
  ...
  // Create keystore with old default path
  ...
  keystoreX.defaultHdPathString = jsonKS.defaultHdPathString;
...
KeyStore.prototype.serialize = function () {
  var jsonKS = {'encSeed': this.encSeed,
		...
		'defaultHdPathString' : this.defaultHdPathString,
...
//Putting hdPath as optional argument after callback is a bit ugly,
//but makes integration of the new default path less of a hassle
//for users
KeyStore.prototype.signTransaction = function (txParams, callback, hdPathString) {
  ...
  this.passwordProvider( function (err, password) {
    ...
    KeyStore.deriveKeyFromPassword(password, function (err, pwDerivedKey) {
      var signedTx = signing.signTx(self, pwDerivedKey, rawTx, signingAddress, hdPathString);
