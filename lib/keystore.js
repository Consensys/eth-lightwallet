...
KeyStore.prototype._generatePrivKeys = function(pwDerivedKey, n, hdPathString) {
  ...
  for (var i = 0; i < n; i++){
    ...
    var encPrivKey = KeyStore._encryptKey(key.privateKey.toString(), pwDerivedKey);
...
KeyStore.prototype.exportPrivateKey = function (address, pwDerivedKey, hdPathString) {
  ...
  var strippedAddr = strip0x(address);
...
KeyStore.prototype._asymEncryptRaw = function (msgUint8Array, myPubKey, theirPubKey, pwDerivedKey, hdPathString) {
...
KeyStore.prototype._asymDecryptRaw = function (encMsg, theirPubKey, myPubKey, pwDerivedKey, hdPathString) {
...
KeyStore.prototype.asymEncryptString = function (msg, myPubKey, theirPubKey, pwDerivedKey, hdPathString) {

  var messageUInt8Array = nacl.util.decodeUTF8(msg);

  return this._asymEncryptRaw(messageUInt8Array, myPubKey, theirPubKey, pwDerivedKey, hdPathString);

}

KeyStore.prototype.asymDecryptString = function (encMsg, theirPubKey, myPubKey, pwDerivedKey, hdPathString) {

  var cleartext = this._asymDecryptRaw(encMsg, theirPubKey, myPubKey, pwDerivedKey, hdPathString);
...
KeyStore.prototype.multiEncryptString = function (msg, myPubKey, theirPubKeyArray, pwDerivedKey, hdPathString) {
  ...
  for (var i=0; i<theirPubKeyArray.length; i++) {
    ...
    var encSymKey = this._asymEncryptRaw(symEncryptionKey, myPubKey, theirPubKeyArray[i], pwDerivedKey, hdPathString);
...
KeyStore.prototype.multiDecryptString = function (encMsg, theirPubKey, myPubKey, pwDerivedKey, hdPathString) {

  var symKey = false;
  for (var i=0; i < encMsg.encryptedSymKey.length; i++) {
    var result = this._asymDecryptRaw(encMsg.encryptedSymKey[i], theirPubKey, myPubKey, pwDerivedKey, hdPathString)
    if (result !== false) {
      symKey = result;
      break;
    }
...
KeyStore.prototype.signTx = function (rawTx, pwDerivedKey, signingAddress, hdPathString) {
...
KeyStore.prototype.signTransaction = function (txParams, callback) {
  ...
  this.passwordProvider( function (err, password) {
    KeyStore.deriveKeyFromPassword(password, function (pwDerivedKey) {
      var signedTx = self.signTx(rawTx, pwDerivedKey, signingAddress, this.defaultHdPathString);
