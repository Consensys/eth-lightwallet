...
var KeyStore = function(mnemonic, pwDerivedKey, hdPathString) {
  ...
  if ( (typeof pwDerivedKey !== 'undefined') && (typeof mnemonic !== 'undefined') ){
    ...
    this.encSeed = encryption._encryptString(paddedSeed, pwDerivedKey);
    ...
    this.encHdRootPriv = encryption._encryptString(hdRoot, pwDerivedKey);

    var hdRootKey = new bitcore.HDPrivateKey(hdRoot);
    var hdPath = hdRootKey.derive(hdPathString).xprivkey;
    pathKsData.encHdPathPriv = encryption._encryptString(hdPath, pwDerivedKey);
  }
};
...
KeyStore.prototype.addHdDerivationPath = function (hdPathString, pwDerivedKey, info) {
  ...
  var hdRoot = encryption._decryptString(this.encHdRootPriv, pwDerivedKey);
  var hdRootKey = new bitcore.HDPrivateKey(hdRoot);
  var hdPath = hdRootKey.derive(hdPathString).xprivkey;

  this.ksData[hdPathString] = {};
  this.ksData[hdPathString].info = info;
  this.ksData[hdPathString].encHdPathPriv = encryption._encryptString(hdPath, pwDerivedKey);
...
KeyStore.prototype._generatePrivKeys = function(pwDerivedKey, n, hdPathString) {
  ...
  var hdRoot = encryption._decryptString(this.ksData[hdPathString].encHdPathPriv, pwDerivedKey);
  var keys = [];
  for (var i = 0; i < n; i++){
    var key = new bitcore.HDPrivateKey(hdRoot).derive(this.ksData[hdPathString].hdIndex++);
    var encPrivKey = encryption._encryptKey(key.privateKey.toString(), pwDerivedKey);
...
KeyStore.prototype.getSeed = function (pwDerivedKey) {
  var paddedSeed = encryption._decryptString(this.encSeed, pwDerivedKey);
...
KeyStore.prototype.exportPrivateKey = function (address, pwDerivedKey, hdPathString) {
  ...
  var privKey = encryption._decryptKey(encPrivKey, pwDerivedKey);
