...
KeyStore.prototype.loadAddress = function (serAddr, hdPathString) {
  ...
  try {
    // already exists
    this._checkAddressExists(jsonAddr.address, hdPathString);
    console.warn('Address ' + jsonAddr.address + ' already exists in keystore at ' + hdPathString);

    return ;

  } catch(err) {
    // load new address
    this.ksData[hdPathString].encPrivKeys[jsonAddr] = jsonAddr.encPrivKey;
    this.ksData[hdPathString].addresses.push(jsonAddr.address);
  }
}
