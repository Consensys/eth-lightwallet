 ...
 *      "address": "<address>",
 *      "encPrivKey": {
 *        "key": "<the encrypted private key of this address>",
 *        "nounce": "<nounce used to encrypt>"
 *      }
 *   }
 */
KeyStore.prototype.dumpAddress = function (address, hdPathString) {
...
KeyStore.prototype.loadAddress = function (serAddr, hdPathString) {

  var jsonAddr = JSON.parse(serAddr);
  var addr = jsonAddr.address;

  if (hdPathString === undefined) {
    hdPathString = this.defaultHdPathString;
  }

  this._validateAddressSerSchema(jsonAddr);
  this._checkHdPathPurpose(hdPathString, 'sign');

  try {
    // already exists
    this._checkAddressExists(addr, hdPathString);
    console.warn('Address ' + addr + ' already exists in keystore at ' + hdPathString);

    return ;

  } catch(err) {
    // load new address
    this.ksData[hdPathString].encPrivKeys[addr] = jsonAddr.encPrivKey;
    this.ksData[hdPathString].addresses.push(addr);
  }
}

KeyStore.prototype._validateAddressSerSchema = function (jsonAddr) {
  if (typeof jsonAddr === 'string') {
    jsonAddr = JSON.parse(jsonAddr);
  }

  if (!jsonAddr.address) {
    throw new Error('"address" is missing from the address serialization');
  }

  if (!jsonAddr.encPrivKey) {
    throw new Error('"key" is missing from the address serialization');
  }

  return true;
}

KeyStore.prototype._checkHdPathPurpose = function (hdPathString, purpose) {
  if (this.ksData[hdPathString].info.purpose !== purpose) {
    throw new Error('Addresses not defined when purpose is not "' + purpose + '"');
  }
}

KeyStore.prototype._checkAddressExists = function (address, hdPathString) {
  address = strip0x(address);
