...
function addressToPublicEncKey(keystore, pwDerivedKey, address) {
  ...
  var pubKeyUInt8Array = nacl.box.keyPair.fromSecretKey(privKeyUInt8Array)
    .publicKey;
  return nacl_encodeHex(pubKeyUInt8Array);
}

function _asymEncryptRaw(
  keystore,
  pwDerivedKey,
  msgUint8Array,
  myAddress,
  ...
  var output = {
    ...
    ciphertext: naclUtils.encodeBase64(encryptedMessage)
  };

  return output;
}

function _asymDecryptRaw(
  keystore,
  pwDerivedKey,
  encMsg,
  theirPubKey,
  ...
  var nonce = naclUtils.decodeBase64(encMsg.nonce);
  var ciphertext = naclUtils.decodeBase64(encMsg.ciphertext);
...
var asymEncryptString = function(
  keystore,
  pwDerivedKey,
  msg,
  myAddress,
  ...
  var messageUInt8Array = naclUtils.decodeUTF8(msg);

  return _asymEncryptRaw(
    keystore,
    pwDerivedKey,
    messageUInt8Array,
    myAddress,
...
var asymDecryptString = function(
  keystore,
  pwDerivedKey,
  encMsg,
  theirPubKey,
  ...
  if (cleartext === false) {
    ...
    return naclUtils.encodeUTF8(cleartext);
  }
};

var multiEncryptString = function(
  keystore,
  pwDerivedKey,
  msg,
  myAddress,
  ...
  var symEncMessage = nacl.secretbox(
    messageUInt8Array,
    symNonce,
    symEncryptionKey
  );
  ...
  output.symNonce = naclUtils.encodeBase64(symNonce);
  output.symEncMessage = naclUtils.encodeBase64(symEncMessage);
  output.encryptedSymKey = encryptedSymKey;

  return output;
};

var multiDecryptString = function(
  keystore,
  pwDerivedKey,
  encMsg,
  theirPubKey,
  ...
  for (var i = 0; i < encMsg.encryptedSymKey.length; i++) {
    var result = _asymDecryptRaw(
      keystore,
      pwDerivedKey,
      encMsg.encryptedSymKey[i],
      theirPubKey,
      myAddress
    );
    if (!!result !== false) {
  ...
  if (symKey === false) {
    ...
    var symNonce = naclUtils.decodeBase64(encMsg.symNonce);
    var symEncMessage = naclUtils.decodeBase64(encMsg.symEncMessage);
    var msg = nacl.secretbox.open(symEncMessage, symNonce, symKey);

    if (msg === false) {
      return false;
    } else {
      return naclUtils.encodeUTF8(msg);
    }
  }
};
